...
import CalendarSidebar from '@/components/modals/CalendarSidebar';
...
import { Task, Meeting, Note, AgendaView, RecurringEditScope } from '@/types';
import {
  addMeeting,
  editMeeting,
  deleteMeeting,
  editRecurringMeeting,
...
const Dashboard: React.FC<DashboardProps> = () => {
  ...
  } = useAgenda(selectedDate, agendaView);
  ...
  const { viewMode, listDensity, setViewMode, setListDensity } =
    useViewPreference();
  ...
  const handleMeetingSave = async (
    ...
    try {
      if (meetingToEdit) {
        const isRecurring =
          meetingToEdit.isRecurringInstance ||
          meetingToEdit.isRecurringBase;
        ...
        >
          <div className={`flex gap-6 ${isMobile ? 'min-h-screen' : 'h-full'}`}>
            ...
            >
              ...
              <div
                className={`flex-1 ${
                  isMobile ? 'flex flex-col' : 'flex flex-col lg:grid lg:grid-cols-12'
                } gap-6 ${isMobile ? '' : 'min-h-0'}`}
              >
                {/* Tasks Section */}
                <div
                  className={`${
                    isMobile ? 'w-full' : 'lg:col-span-8'
                  } flex flex-col ${isMobile ? '' : 'min-h-0'}`}
                >
                  ...
                  <div className="flex-1 overflow-y-auto scrollbar-hide pb-4">
                    {activeSection === 'tasks' || activeSection === 'dashboard' ? (
                      ...
                      <div className="pro-glass pro-rounded-lg p-8 text-center">
                        ...
                        <p className="text-white/40 text-sm">
                          This feature is coming soon! We&apos;re working hard to bring
          ...
          <button
            ...
            className={`fixed ${
              isMobile ? 'bottom-20 right-6' : 'bottom-6 left-6'
            } w-14 h-14 bg-gradient-to-r from-blue-600 to-purple-600 rounded-full flex items-center justify-center text-white shadow-lg hover:scale-110 transition-transform duration-200 z-50`}
          >
