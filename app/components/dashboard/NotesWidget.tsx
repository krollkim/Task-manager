...
const NotesWidget: React.FC<NotesWidgetProps> = ({
  ...
  const { notes, createNote, updateNote, deleteNote, togglePin, refetch } =
    useNotes();
          ...
          sortedNotes.map((note) => (
            <div
              ...
              className={`p-3 pro-card-gradient pro-rounded group transition-all duration-300 cursor-pointer relative ${
                menuOpen && selectedNoteId === note._id
                  ? ''
                  : 'hover:scale-[1.01]'
              }`}
            >
              <div className="flex items-start justify-between">
                ...
                <button
                  ref={(el) => {
                    if (el) buttonRefs.current[note._id] = el;
                  }}
                  onClick={(e) => {
                    e.stopPropagation();
        ...
        createPortal(
          <>
            ...
            >
              <button
                onClick={() =>
                  selectedNoteId && handlePinNote(selectedNoteId)
                }
              ...
              >
                ...
                {notes.find((n) => n._id === selectedNoteId)?.pinned
                  ? 'Unpin'
                  : 'Pin'}{' '}
                Note
              </button>
              <button
                onClick={() =>
                  selectedNoteId && handleEditNote(selectedNoteId)
                }
