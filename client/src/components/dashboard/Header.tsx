  ...
  return (
    <header className="pro-sidebar-gradient pro-rounded-lg pro-shadow h-16 flex items-center justify-between px-4 md:px-6 mb-6 pro-fade-in">
      ...
      <div className="flex items-center space-x-2 md:space-x-4 flex-1">
        {isMobile && (
          <IconButton onClick={onMenuClick} className="text-white hover:bg-white/10 transition-colors duration-200">
            <MenuOutlined />
          </IconButton>
        )}
        ...
        <div className="relative flex-1 max-w-lg">
          <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            <SearchOutlined className="text-white/60 text-lg" />
          </div>
          <input
            type="text"
      ...
      <div className="flex items-center space-x-1 md:space-x-3">
        ...
        <IconButton className="text-white hover:bg-white/10 transition-colors duration-200 hidden sm:flex">
          <NotificationsOutlined fontSize={isMobile ? 'small' : 'medium'} />
        </IconButton>

        {/* Settings - Hide on very small screens */}
        <IconButton className="text-white hover:bg-white/10 transition-colors duration-200 hidden sm:flex">
          <SettingsOutlined fontSize={isMobile ? 'small' : 'medium'} />
        </IconButton>
